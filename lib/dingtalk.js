...
class DingTalk {
   ...
   */
  getUserInfo(code) {
    return this.httpUtil.get('/user/getuserinfo', {
      code,
    }, [ 'userid', 'deviceId', 'is_sys', 'sys_level' ]);
  }
   ...
   */
  userInfo(userid) {
    if (!userid) throw new Error('misssing user_id');
    return this.httpUtil.get('/user/get', { userid });
  }

  /**
   * 根据 unionid 获取成员的 UserId
   *
   * @param {String} unionid - 成员 unionid
   *
   * @return {String} UserId
   *
   * @memberOf DingTalk
   */
  userIdByUnionid(unionid) {
    if (!unionid) throw new Error('misssing user_id');
    return this.httpUtil.get('/user/getUseridByUnionid', { unionid }, 'userid');
  }
